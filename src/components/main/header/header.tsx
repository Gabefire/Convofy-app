...
export default function Header() {
	...
	return (
		<div className="fixed top-0 flex dark:bg-neutral-700 bg-white w-full justify-between h-14 min-h-14 dark:text-white pl-2 pr-2 items-center border-b-neutral-400 border-b dark:border-none">
			...
			<div className="min-w-64 w-64 sm:flex self-center rounded-full p-1 dark:bg-neutral-500 bg-neutral-300 h-10 hidden">
				<Magnify
					className="size-4 ml-1 mr-1 self-center"
					fill={enabled ? "white" : "black"}
				/>
				<input
					type="text"
					className="bg-inherit focus:outline-none focus:ring-0 w-52"
					ref={searchBarRef}
					onChange={(e) => {
						if (searchTerm) {
			...
			<div className="flex items-center sm:gap-4 gap-2">
				...
				<Link to="/create-forum">
					<button
						...
						sm:text-lg text-3xl sm:border-solid dark:border-white dark:border bg-neutral-300 dark:bg-transparent"
						title="Create forum"
					>
